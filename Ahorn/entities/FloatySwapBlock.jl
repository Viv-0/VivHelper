module VivHelperFloatySwapBlock
using ..Ahorn, Maple

@mapdef Entity "VivHelper/FloatySwapBlock" FloatySwapBlock(x::Integer, y::Integer, width::Integer=16, height::Integer=16)
...
const placements = Ahorn.PlacementDict(
    "Floaty Swap Block ($(uppercasefirst(theme)), Viv's Helper)" => Ahorn.EntityPlacement(
        FloatySwapBlock,
        "rectangle",
        Dict{String, Any}(
            "theme" => theme
...
Ahorn.editingOptions(entity::FloatySwapBlock) = Dict{String, Any}(
    "theme" => Maple.swap_block_themes
)

Ahorn.nodeLimits(entity::FloatySwapBlock) = 1, 1

Ahorn.minimumSize(entity::FloatySwapBlock) = 16, 16
Ahorn.resizable(entity::FloatySwapBlock) = true, true

function Ahorn.selection(entity::FloatySwapBlock)
    x, y = Ahorn.position(entity)
...
function getTextures(entity::FloatySwapBlock)
...
function Ahorn.renderSelectedAbs(ctx::Ahorn.Cairo.CairoContext, entity::FloatySwapBlock, room::Maple.Room)
...
function Ahorn.renderAbs(ctx::Ahorn.Cairo.CairoContext, entity::FloatySwapBlock, room::Maple.Room)
    sprite = get(entity.data, "sprite", "block")

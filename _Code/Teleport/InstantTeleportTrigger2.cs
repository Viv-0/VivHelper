...
namespace VivHelper.Triggers {
    ...
    public class InstantTeleportTrigger1Way : Trigger {
        ...
        public override void Awake(Scene scene) {
            ...
            if ((scene as Level).Session.DoNotLoad.Contains(ID)) {Logger.Log("VivHelper", "Teleporter with ID `" + ID + "` is getting removed");
        ...
        public IEnumerator TeleportMaster(Player player) {
            ...
            if (VivHelperModule.Session.TeleportState || level == null ||
                VivHelperModule.Session.TeleportAction != null) {
                yield break;
            }
            if (customDelay > 0)

...
[CustomEntity(new string[] { "VivHelper/CelsiusGrowBlock" })]
[Tracked(false)]
public class GrowBlock : Solid {
    ...
    public override void Update() {
        ...
        if (level != null && !string.IsNullOrEmpty(flag)) {
            bool num = level.Session?.GetFlag(flag) ?? true;
            bool visible = num;
            Collidable = num;
            Visible = visible;
        }
    ...
    public void Resize() {
        ...
        if (entity != null) {
            ...
            float num2 = Math.Max(width, height) + (float) baseDist;
            num2 *= num2;
            float val = Math.Max(0f, num - num2) / (float) factor;
            ...
            lightOccludeBounds.SetValue(lightOcclude, new Rectangle((int) ((1f - alphaB) * 0.5f * width + offset.X), (int) ((1f - alphaB) * 0.5f * height + offset.Y), (int) (width * alphaB), (int) (height * alphaB)));
    ...
    public override void Render() {
        if (CullHelper.IsRectangleVisible(base.Hitbox.AbsoluteLeft, base.Hitbox.AbsoluteTop, base.Hitbox.Width, base.Hitbox.Height, 4f, (Camera) null) && buffer != null && alphaC > 0f) {

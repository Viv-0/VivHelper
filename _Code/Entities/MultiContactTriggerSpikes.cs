...
namespace VivHelper.Entities {
    ...
    {
    "VivHelper/MultiContactTriggerSpikesUp = LoadUp",
    "VivHelper/MultiContactTriggerSpikesDown = LoadDown",
    "VivHelper/MultiContactTriggerSpikesLeft = LoadLeft",
    "VivHelper/MultiContactTriggerSpikesRight = LoadRight"
    })]
    public class MultiContactTriggerSpikes : Entity {
        ...
        private struct SpikeInfo {
            public MultiContactTriggerSpikes Parent;
            ...
            public void Update() {
                if (Triggered) {
                    ...
                    if (Parent.contactLimit > contactCount) {
                        Triggered = PlayerCheck();
                        if (!PrevTriggered && Triggered) { contactCount++; }
                    } else {
                        if (DelayTimer > 0f) {
        ...
        public static Entity LoadUp(Level level, LevelData levelData, Vector2 offset, EntityData entityData) {
            return new MultiContactTriggerSpikes(entityData, offset, Directions.Up);
        }

        public static Entity LoadDown(Level level, LevelData levelData, Vector2 offset, EntityData entityData) {
            return new MultiContactTriggerSpikes(entityData, offset, Directions.Down);
        }

        public static Entity LoadLeft(Level level, LevelData levelData, Vector2 offset, EntityData entityData) {
            return new MultiContactTriggerSpikes(entityData, offset, Directions.Left);
        }

        public static Entity LoadRight(Level level, LevelData levelData, Vector2 offset, EntityData entityData) {
            return new MultiContactTriggerSpikes(entityData, offset, Directions.Right);
        }

        public MultiContactTriggerSpikes(EntityData data, Vector2 offset, Directions dir)
            : this(data.Position + offset, GetSize(data, dir), dir, data.Attr("type", "default"), data.Int("contactLimit", 1)) {
        }

        public MultiContactTriggerSpikes(Vector2 position, int size, Directions direction, string overrideType, int contactLimit)
            : base(position) {
        ...
        public override void Added(Scene scene) {
            ...
            for (int i = 0; i < spikes.Length; i++) {
                ...
                spikes[i].Index = i;
                spikes[i].playerContact = false;
                spikes[i].contactCount = 0;
                switch (direction) {
                    case Directions.Up:

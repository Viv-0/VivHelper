...
namespace VivHelper.Entities {
    [CustomEntity(new string[] { "VivHelper/SolidModifier", "VivHelper/SolidModifier2" })]
    public class SolidModifier : Entity {
        ...
        public SolidModifier(EntityData data, Vector2 offset) : base(data.Position + offset) {
            ...
            if (data.Name == "VivHelper/SolidModifier2") {
                cornerboost = -data.Int("cornerBoostBlock");
                retainMomentumThroughCornerBoost = cornerboost == 2 || data.Bool("RetainWallSpeed", defaultValue: true);
            } else {
                cornerboost = data.Int("CornerBoostBlock");
            }
        ...
        public override void Awake(Scene scene) {
            ...
            foreach (Solid solid in CollideAll<Solid>()) {
                ...
                if (t == null) { throw new Exception("Please report this to Viv on Discord @vivianlonging asap, thank you"); } //Added this get because of a very bizarre crash that occurred once ever.

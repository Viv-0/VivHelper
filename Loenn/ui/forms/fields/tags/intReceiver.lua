...
local intReceiver = {}
intReceiver.fieldType = "VivHelper.tagIntReceiver"
...
function intReceiver.getElement(name, value, fieldInformation)
    local formField = {}
...
-- We only need the tags to have values from map *if* we're editing one of the tagged elements, so this is fine to do here.
    ...
    dropdown._capReference = function() return tagHelper.getNextInteger(fieldInformation._vivh_class) - 1 end
    ...
    dropdown.tooltipText = string.format(tostring(language.ui.VivHelper.intReceiver.tooltip), tostring(language.tags.VivHelper[fieldInformation._vivh_class].largedetail))

...
local receiver = {}
receiver.fieldType = "VivHelper.tagReceiver"
...
function receiver.getElement(name, value, fieldInformation)
    local formField = {}
...
-- We only need the tags to have values from map *if* we're editing one of the tagged elements, so this is fine to do here.
    ...
    dropdown.tooltipText = string.format(tostring(language.ui.VivHelper.receiver.tooltip), tostring(language.tags.VivHelper[fieldInformation._vivh_class].largedetail))

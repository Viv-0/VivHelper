...
local function getSprite(entity, bg)
    ...
    if entity.ref then 
        ...
        if bg then
            ...
            s = s:gsub("gf/","gb/",1)
            s = s:reverse()
        end -- get last occurrence of /fg and replace with /bg if background sprite, yes this is actually how you do it and it's fucking stupid
    else
    ...
    if filerefCache[s .. "00"] then 
        return filerefCache[s.."00"]
    else
        local ret = drawableSprite.fromTexture(s .. "00", entity)
        if ret then return ret end
    end
    if filerefCache[s] then return filerefCache[s]
    else
...
function customSpinner.sprite(room, entity)
    ...
    local color = vivUtil.oldGetColorTable(c)
